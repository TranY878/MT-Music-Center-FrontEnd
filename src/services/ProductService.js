...
export const getAllProduct = async (search, limit) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-all?filter=name&search=${search}&limit=${limit}`);
...
export const addDiscount = async (discountData, access_token) => {
    const res = await axiosJWT.post(`${process.env.REACT_APP_API_URL}/product/add-discount`, discountData, {
        headers: {
            token: `Bearer ${access_token}`,
        }
    });
